...
router.get("/admindashboard", function (req, res) {
	if (req.session.adminemail) {
		...
		pool.query ('select count(*) as total , count(case when visit = "Yes" then 1 end) as completed, count (case when visit = "No" then 1 end) as pending, count ( case when mcondition = "Emergency" then 1 end) as emergency , count (case when mcondition = "Critical" then 1 end ) as critical from appointment' , (err,obj)=>{
			if (err){
				console.log (err);
				res.render("admindashboard", { adminemail: req.session.adminemail, image: req.session.photu, total : 0 , doctor : 0 , completed : 0 , pending :0, emergency : 0 , critical : 0 , report : 0});	
			}

			else{
				pool.query ('select count(*) as doc from doctor' , (err2,obj2) =>{
					if (err2){
						console.log (err2);
						res.render("admindashboard", { adminemail: req.session.adminemail, image: req.session.photu, total : 0 , doctor : 0 , completed : 0 , pending :0, emergency : 0 , critical : 0 , report : 0});	
					}
						...
						pool.query ('select count(*) as report from reports' , (err3 , obj3)=>{
							if (err3){
								console.log (err3);
								res.render("admindashboard", { adminemail: req.session.adminemail, image: req.session.photu, total : 0 , doctor : 0 , completed : 0 , pending :0, emergency : 0 , critical : 0 , report : 0});	
								...
								//  doctor : (ob2.length == 0 ? 0 : obj2[0]['doc'] ), 
								//  completed : (obj.length == 0 ? 0 :  obj[0]['completed'] ),
								//  pending :( obj.length == 0 ? 0 : obj[0]['pending'] ),
								//   emergency :(obj.length == 0 ? 0 :  obj[0]['emergency'] ), 
								//   critical : (obj.length == 0 ? 0 : obj[0]['critical'] ), 
								// report :(obj3.length == 0 ? 0 :  obj3[0]['report'])
								res.render("admindashboard", { adminemail: req.session.adminemail, image: req.session.photu, total : (obj.length == 0 ? 0 : (obj[0]['total']) ) , doctor : obj2[0]['doc'] , completed : obj[0]['completed'] , pending : obj[0]['pending'] , emergency : obj[0]['emergency'] , critical : obj[0]['critical'] , report : obj3[0]['report']});	
...
router.get("/showproducts", function (req, res) {
	if (req.session.adminemail) {
		...
		res.render("showproducts", { adminemail: req.session.adminemail });
		logger.customLogger.log("info", mes);
	} else {
								...
								if (err3) {
									...
									pool.query("SELECT * FROM `users`", (err4, obj4) => {
										if (err4) {
											...
											pool.query(
												"SELECT DISTINCT `month`,`year`,`email` FROM `schedule` WHERE status = ?",
...
<head>
    ...
    <title>Clinic Email Template</title>
    <style>
        ...
        .header {
            background: linear-gradient(135deg, #2a9d8f, #27a69d);
            padding: 30px;
            text-align: center;
            color: #ffffff;
        }
...
<body>
    ...
    <div class="email-container">
        <!-- Header Section -->
        <div class="header">
            <img src="https://media.istockphoto.com/id/1139549801/vector/stethoscope-heart-icon.jpg?s=612x612&w=0&k=20&c=qEJ7fFxWkok8j7FYYj4NwAlHSgqsw-azZz7c3IQJ4KI="
                alt="Clinic Icon" class="icon">
        ...
        <!-- Content Section -->
        <div class="content">
            <h2>Hello ${text.name},</h2>
            <p>Thank you for choosing our clinic for your healthcare needs. We are dedicated to providing you with the
                best care. Here are your appointment details:</p>

            <p><strong>🧍‍♀️ Token No.:</strong> ${result.insertId}</p>
            <p><strong>👩🏻 Patient Name:</strong> ${req.body.name}</p>
            <p><strong>💳 Aadhar No.:</strong> ${req.body.aadhar}</p>
            <p><strong>📅 Appointment Date:</strong> ${text.date}</p>
            <p><strong>👨‍⚕️ Doctor:</strong> Dr. ${obj[0].name}</p>
            <p><strong>📍 Condition:</strong> ${text.condition}</p>

            <p>Please arrive 10 minutes early. If you need to reschedule, contact us at nrtech@nr.com .</p>

            <!-- Call-to-Action Button -->
            <a href="http://localhost:3002" class="button">View Appointment</a>

            <p>We look forward to serving you. Stay healthy!</p>
            <p>Warm regards,</p>
            <p>The NR Technologies Team</p>
        </div>

        <!-- Footer Section -->
        <div class="footer">
            <p>&copy; 2024 NR TECHNOLOGIES. All rights reserved.</p>
...
router.post("/adddoctor", (req, res) => {
	...
	if (req.session.adminemail) {
		...
		pool.query(
			"INSERT INTO `doctor` (email , name, type, password , speciality) VALUES (?,?,?,?,?)",
			[text.email, text.name, "Doctor", password, text.speciality],
			(err, obj) => {
				if (err) {
					console.log(err);
...
<body>
    ...
    <div class="email-container">
        ...
        <!-- Content Section -->
        <div class="content">
            <h2>Dear Dr. ${text.name},</h2>
            <p>We are delighted to inform you that your registration has been successfully completed. Thank you for joining
                our esteemed team at NR Technologies. Your expertise and dedication to patient care will be a valuable
                addition to our clinic.</p>

            <p><strong>Registration Details:</strong></p>
            <ul>
                <li><strong>📧 Email:</strong> ${text.email}</li>
                <li><strong>🔐 Password:</strong> ${p}</li>
                <li><strong>🏢 Specialization:</strong> ${text.speciality}</li>
                
            </ul>

            <p>You can now log in to your account, manage appointments, and access our exclusive resources for healthcare
                professionals.</p>

            <!-- Call-to-Action Button -->
            <a href="http://localhost:3002/admin/adminlogin" class="button">View Appointment</a>
...
`
					...
					var mailOptions = {
						from: "rahulrawat31r@gmail.com",
						to: text.email,
						subject: "Doctor Registration Successfull !",
						html: html,
					};
...
router.post("/adduserform", (req, res) => {
	...
	if (req.session.adminemail) {
		let password = Math.floor(1000 + Math.random() * 9000);
		let pswd = aes256.encrypt(passKey, password.toString());
		pool.query("SELECT * FROM `patients` WHERE email = ?", [text.useremail], (err3, obj3) => {
			if (err3) {
				...
				if (obj3.length != 0) {
					...
					pool.query(
						"INSERT INTO `patients` (email,name,age,password,aadhar,gender,mob) VALUES (?,?,?,?,?,?,?)",
						[text.email, text.name, text.age, pswd, text.aadhar, text.gender, text.mob],
						(err, obj) => {
							if (err) {
								console.log(err);
...
<div class="email-container">
	...
	<div class="content">
		 <h2>Hello ${text.name},</h2>
            <p>We are thrilled to have you join our clinic! Your registration has been successfully completed, and we are
                excited to assist you in achieving your health and wellness goals.</p>

		<p><strong>Registration Details:</strong></p>
		<ul>
			<li><strong>📧 Email:</strong> ${text.email}</li>
			<li><strong>🔐 Password:</strong> ${password}</li>
			<li><strong>🎂 Age:</strong> ${text.age}</li>
			<li><strong>📞 Mobile:</strong> ${text.mob}</li>
			<li><strong>🔑 Aadhar:</strong> ${text.aadhar}</li>

		</ul>

		 <p>You can now log in to your account, book appointments, access your medical records, and connect with our
                expert healthcare providers.</p>

		<!-- Call-to-Action Button -->
		<a href="http://localhost:3002/" class="button">View Appointment</a>

		 <p>If you have any questions or need assistance, our support team is here to help you. Simply reply to this
                email or contact us directly at NR TECH.</p>
		<p>We look forward to being your trusted healthcare partner!</p>
		<p>Warm regards,</p>
		<p>The NR Technologies Team</p>
	</div>
...
router.get("/changepermission", (req, res) => {
	pool.query(
		"UPDATE `users` SET permission = ? WHERE email = ?",
		[req.query.permission, req.query.email],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.post("/addchemist", (req, res) => {
	if (req.session.adminemail) {
		...
		pool.query(
			"INSERT INTO `chemist` (name,contact,area) VALUES(?,?,?)",
			[text.chemistname, text.phone, text.area],
			(err2, obj2) => {
				if (err2) {
...
router.post("/addstockist", (req, res) => {
	if (req.session.adminemail) {
		...
		pool.query(
			"INSERT INTO `stockist` (name,contact,area) VALUES(?,?,?)",
			[text.stockistname, text.phone, text.area],
			(err2, obj2) => {
				if (err2) {
					console.log(err);
...
router.get("/getpendingRequests", (req, res) => {
	pool.query(
		"SELECT DISTINCT `email` FROM `schedule` WHERE status = ?",
		["Pending For Approval"],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.get("/getpendingRequestsYear", (req, res) => {
	...
	pool.query(
		"SELECT DISTINCT `year` FROM `schedule` WHERE email = ? AND status = ? ",
		[req.query.email, "Pending For Approval"],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.get("/getpendingRequestsMonth", (req, res) => {
	...
	pool.query(
		"SELECT DISTINCT `month` FROM `schedule` WHERE email = ? AND status = ? AND year = ? ",
		[text.email, "Pending For Approval", text.year],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.get("/getpendingRequestsData", (req, res) => {
	if (req.session.adminemail) {
		...
		pool.query(
			...
			[text.email, "Pending For Approval", text.year, text.month, text.start, text.end],
			(err, obj) => {
				if (err) {
					console.log(err);
...
router.get("/approvingStatus", (req, res) => {
	if (req.session.adminemail) {
		...
		if (text.status == 1) {
			pool.query(
				"UPDATE `schedule` SET status = ? WHERE email = ? AND year = ? AND month = ? AND date >= ? AND date <= ? AND NOT status = ?",
				["Approved", text.email, text.year, text.month, text.start, text.end, "Declined"],
				(err, obj) => {
					if (err) {
						console.log(err);
			...
			pool.query(
				"UPDATE `schedule` SET status = ? WHERE email = ? AND year = ? AND month = ? AND date >= ? AND date <= ?",
				["Declined", text.email, text.year, text.month, text.start, text.end],
				(err, obj) => {
					if (err) {
						console.log(err);
						let mes = `| Request -> /admin/approvingStatus | IP -> ${req.ip} | Error Getting Data from db | Admin -> ${req.session.adminemail} | `;
...
router.get("/fetchSchedule", (req, res) => {
	if (req.session.adminemail) {
		...
		res.render("adminScheduleDetails", {
			adminemail: req.session.adminemail,
			data: [],
			title: "Get Schedule",
		});
...
router.get("/getScheduleMonth", (req, res) => {
	...
	pool.query(
		"SELECT DISTINCT `month` FROM `schedule` WHERE email = ? AND year = ? AND status = ? ",
		[text.email, text.year, text.status],
		(err, obj) => {
			if (err) {
				console.log(err);
				...
				if (err) {
					...
					if (text.checking == "no") {
						res.render("adminScheduleDetails", {
							adminemail: req.session.adminemail,
							data: obj,
							title: text.useremail + " - " + months[text.monthuser - 1],
						});
					} else {
						res.render("adminScheduleDetailswithDcr", {
							adminemail: req.session.adminemail,
							data: obj,
							title: text.useremail + " - " + months[text.monthuser - 1],
						});
...
router.get("/checkDateSchedule", (req, res) => {
	if (req.session.adminemail) {
		...
		pool.query(
			...
			[text.date, text.year, text.month, text.email, "Approved"],
			(err, obj) => {
				if (err) {
					console.log(err);
...
router.get("/getDCRvisits", (req, res) => {
	if (req.session.adminemail) {
		...
		pool.query(
			"SELECT * FROM `schedule` WHERE date = ? AND year = ? AND month = ? AND email = ? AND status = ?",
			[x.getDate(), x.getFullYear(), x.getMonth() + 1, text.email, "Approved"],
			(err, obj) => {
				if (err) {
					console.log(err);
...
router.post("/addadminform", (req, res) => {
	const text = req.body;
	console.log(text);
	...
	pool.query(
		"INSERT INTO `doctor` (email , name, type, speciality, password) VALUES (?,?,?,?,?)",
		[text.email, text.name, text.role, "None", pswd],
		(err, obj) => {
			if (err) {
				console.log(err);
				...
				if (text.role == "Doctor") {
					pool.query(
						"Insert into speciality (email, speciality) values (?,?)",
						[text.email, text.speciality],
						(err, obj) => {
							if (err) {
								console.log(err);
...
router.get("/checkPswd", (req, res) => {
	if (req.session.adminemail) {
		pool.query(
			"SELECT `adminpassword` FROM `admin` WHERE `adminemail` =?",
...
router.get("/getdoctorS/:speciality", (req, res) => {
	pool.query(
		"select d.name, d.email from doctor d where d.speciality = ?",
...
router.get("/getAppointments", (req, res) => {
	pool.query(
		"select a.* , d.name as dname, d.speciality as specs from appointment a, doctor d where d.email = a.doctor ",
		[req.session.email],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.get("/getAppointmentsDoc/:condition/:date/:type", (req, res) => {
	if (req.session.type == "Doctor") {
		let query =
			"select a.* , d.name as dname from appointment a, doctor d where d.email = a.doctor and a.doctor = ? and a.visit =? and a.adate = ?";
		if (req.params.condition != "All") {
			query =
				"select a.* , d.name as dname from appointment a, doctor d where d.email = a.doctor and a.doctor = ? and a.visit = ? and a.adate = ? and a.mcondition = ?";
		}

		pool.query(
			query,
			[req.session.adminemail, req.params.type, req.params.date, req.params.condition],
			(err, obj) => {
				if (err) {
					console.log(err);
					res.send([]);
		...
		let query =
			"select a.* , d.name as dname from appointment a, doctor d where d.email = a.doctor and a.visit = ? and a.adate = ?";
		if (req.params.condition != "All") {
			query =
				"select a.* , d.name as dname from appointment a, doctor d where d.email = a.doctor and a.visit = ? and a.adate = ? and a.mcondition = ?";
		}
...
/* Getting the patients past records */

router.get("/getpastrecords/:token", (req, res) => {
	const token = req.params.token;
	console.log("jai");

	pool.query(
		"select a.name, a.token , a.mcondition , d.name as dname , d.speciality as dspeciality , a.adate from appointment a , doctor d where d.email = a.doctor and a.email = (select email from appointment where token = ?)",
		[token],
		(err, obj) => {
			if (err) {
				console.log(err);
...
router.get("/updatePatient/:value/:token", (req, res) => {
	pool.query(
		"update appointment set visit = ? where token = ?",
		[req.params.value, req.params.token],
		(err, obj) => {
			if (err) {
				console.log(err);
				res.redirect("/admin/appdetails");
			} else {
				res.redirect("/admin/appdetails");
			}
		}
