                ...
                if (err3) {
                  ...
                  pool.query("SELECT * FROM `users`", (err4, obj4) => {
                    if (err4) {
                      ...
                      finobj["users"] = obj4;
                      pool.query("SELECT DISTINCT `month`,`year` FROM `schedule` WHERE status = ? AND email = ?", ["Pending For Approval", req.query.email], (err5, obj5) => {
                        if (err5) {
                          ...
                          pool.query("SELECT DISTINCT `month`,`year` FROM `schedule` WHERE status = ? AND email = ?", ["Approved", req.query.email], (err6, obj6) => {
                            if (err6) {
                              ...
                              finobj["approve"] = obj6;
                              pool.query("SELECT DISTINCT `month`,`year` FROM `schedule` WHERE status = ? AND email = ?", ["Declined", req.query.email], (err7, obj7) => {
                                if (err7) {
...
router.post('/checkuser', function (req, res) {
  ...
  pool.query('select * from admin where email=?', [req.body.email], function (error, result) {
    if (error) {
...
router.get('/showproducts', function (req, res) {
  ...
  if (req.session.email) {
    ...
    pool.query("select * from `users` where email = ?", [req.session.email], (err2, obj2) => {
      if (obj2[0].permission == "YES") {

        let q = "select * from `product` where employeename = '" + obj2[0].email + "'";
        pool.query(q, function (error, result) {
          if (error) {
            ...
            q = "SELECT * FROM `users` WHERE email = '" + req.session.email + "'";
            // console.log (q);

            pool.query(q, (err, obj) => {
              // console.log (obj);
              if (err) {
                ...
                let mes = `| Request -> /showproducts | IP -> ${req.ip} | User products page rendered | User -> ${req.session.email} | `;
                logger.customLogger.log ('info',mes);
                res.render('showproductUser', { data: result, adminemail: req.session.email, username: obj[0].email })
        ...
        res.render('userpermissionproduct.ejs', { adminemail: req.session.email })
      }
    })
      ...
      pool.query('insert into product (doctorname,productname,chemistname,stockistname,date,employeename,workingwith,adder,ip,dcontact,stockistphone,chemistphone) values(?,?,?,?,?,?,?,?,?,?,?,?)', [
        ...
        if (error) {
          ...
          pool.query("SELECT * FROM `users` WHERE email = ?", [req.session.email], (err2, obj2) => {
            ...
            pool.query("UPDATE `users` SET productnum = ? WHERE email = ?", [num, req.session.email], (err3, obj3) => { });
          })
          res.redirect('/showproducts')
        }
      });
    ...
    pool.query('delete from product where productid=?', [req.query.productid], function (error, result) {
      if (error) {
        ...
        pool.query("SELECT * FROM `users` WHERE email = ?", [req.session.email], (err, obj) => {
          ...
          pool.query("UPDATE `users` SET productnum = ? WHERE email = ?", [num, req.session.email], (err2, obj2) => {
            res.redirect('/showproducts')
  ...
  if (req.session.email) {
    pool.query("SELECT * FROM `users` WHERE email = ?", [req.session.email], (err, obj) => {
      if (err) {
        ...
        image = "/images/" + obj[0].img;

        let mes = `| Request -> /getprofile | IP -> ${req.ip} | User profile page rendered | User -> ${req.session.email} | `;
        logger.customLogger.log ('info',mes);
        res.render('profileuser', { img: image, name: obj[0].name, email: obj[0].email, designation: obj[0].designation, product: obj[0].productnum, permission: obj[0].permission });
      }
    })
  }
...
router.post('/addScheduleForm', (req, res) => {
  if (req.session.email) {
    ...
    for (i = 0; i < text.docvisits; i++) {
      ...
      pool.query("INSERT INTO `schedule` (email,month,year,date,status,doctor,time,task,remarks,actdoctor,acttime) VALUES (?,?,?,?,'Pending For Approval',?,?,'NA','NA','NA','NA')", [req.session.email, (date.getMonth() + 1), date.getFullYear(), date.getDate(), doc, time], (err, obj) => {
        if (err) {
...
router.get('/checkschedule', (req, res) => {
  ...
  if (req.session.email) {
    ...
    pool.query("SELECT * FROM `schedule` WHERE email = ? AND date = ? AND year = ? AND month = ? AND NOT status = 'Declined'", [req.session.email, text.date, text.year, text.month], (err, obj) => {
      if (err) {
...
router.get('/getSchedule', (req, res) => {
  if (req.session.email) {
    ...
    logger.customLogger.log ('info',mes);
    res.render('userScheduleDetails', { email: req.session.email, data: [], title: "Get Schedule" });
  }
  ...
  if (req.session.email) {
    ...
    pool.query("SELECT * FROM `schedule` WHERE email = ? AND status = ? AND year = ? AND month = ? AND date >= ? AND date <= ? ORDER BY `date`", [text.useremail, text.approval, text.yearuser, text.monthuser, text.datestartrange, text.dateendrange], (err, obj) => {
      if (err) {
        ...
        if (text.checking == "no") {
          res.render('userScheduleDetails', { email: req.session.email, data: obj, title: t })

        }

        else {
          res.render('userScheduleDetailswithDcr', { email: req.session.email, data: obj, title: t })
...
router.post('/dcrform', (req, res) => {
  if (req.session.email) {
    ...
    for (i = 0; i < parseInt(text.visitStatus); i++) {
      ...
      pool.query("UPDATE `schedule` SET `actdoctor` = ? , `acttime` = ?, `remarks` = ?,task = ? WHERE email = ? AND year = ? AND month = ? AND date = ? AND doctor = ? AND time = ? AND status = ?", [acd, act, text.remarks, text.schStatus, req.session.email, JSON.parse(text.sendData)[0].year, JSON.parse(text.sendData)[0].month, JSON.parse(text.sendData)[0].date, JSON.parse(text.sendData)[i].doctor, JSON.parse(text.sendData)[i].time, "Approved"], (err, obj) => {
        if (err) {
...
router.get('/checkDateSchedule', (req, res) => {
  if (req.session.email) {
    ...
    pool.query("SELECT * FROM `schedule` WHERE date = ? AND year = ? AND month = ? AND email = ? AND status = ?", [text.date, text.year, text.month, req.session.email, "Approved"], (err, obj) => {
      if (err) {
...
router.get('/getDCRvisits', (req, res) => {
  if (req.session.email) {
    ...
    pool.query("SELECT * FROM `schedule` WHERE date = ? AND year = ? AND month = ? AND email = ? AND status = ?", [x.getDate(), x.getFullYear(), (x.getMonth() + 1), req.session.email, "Approved"], (err, obj) => {
      if (err) {
...
router.get ('/checkPswd',(req,res)=>{
  if (req.session.email){
      ...
      pool.query ("SELECT `password` FROM `users` WHERE `email` =?",[req.session.email], (err,obj)=>{
          if (err){
...
router.post ('/changePswdForm',(req,res)=>{
  if (req.session.email){
      ...
      pool.query ("select password from users where email = ?",[req.session.email],(err2, obj2)=>{
        if (err){
          ...
          logger.customLogger.log ('error',mes);
            ...
            if (password == text.oldpswd){
              ...
              pool.query ("UPDATE `users` SET `password` = ? WHERE `email` = ?",[pswd, req.session.email],(err,obj)=>{
                  if (err){
                    ...
                    let message = `Attention `+ req.session.email + `!!
